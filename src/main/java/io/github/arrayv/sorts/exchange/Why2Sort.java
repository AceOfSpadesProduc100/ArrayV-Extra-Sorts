...
*/
final public class Why2Sort extends Sort {
    public Why2Sort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Why 2");
        this.setRunAllSortsName("Why Sort 2");
        this.setRunSortName("Why Sort 2");
    ...
    protected void method(int[] array, int start, int end, int pdir, int itr) {
        ...
        if (itr < 20) {
            Writes.recursion();
            method(array, start, end, -1 * pdir, itr + 1);
            Writes.recursion();
            method(array, start, end, pdir, itr + 1);
        } else {
            pd(array, start, end, -1 * pdir);
